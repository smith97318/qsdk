...
void * wlan_service_request()
{
	...
	wsvc_printf_info("%s: Start the pthread", __func__);
	...
	for (i = 0; i < (int)ARRAY_SIZE(ind_handlers); i++) {
		if (nl_loop_register(ind_handlers[i].ind,
			...
			wsvc_printf_err("Failed to register: %d, %x", i,
					ind_handlers[i].ind);
			rc = -1;
			goto err_nl_loop_init;
		}
	}

	rc = pthread_mutex_init(&(cnss_client.wlps_mutex), NULL);
	if (0 != rc) {
		wsvc_printf_err("Failed to init wlps_mutex, ret %d", rc);
		goto err_nl_loop_init;
	}
...
err_mutex_init:
	ret = pthread_mutex_destroy(&(cnss_client.wlps_mutex));
	if (0 != ret)
		wsvc_printf_err("Failed to destroy wlps_mutex, ret %d", ret);
err_nl_loop_init:
	while (--i >= 0)
...
void wlan_service_release(void)
{
	...
	rc = pthread_mutex_destroy(&(cnss_client.wlps_mutex));
	if (0 != rc)
		wsvc_printf_err("Failed to destroy wlps_mutex, ret %d", rc);
}

void wlan_service_shutdown(void)
{
	pthread_mutex_lock(&(cnss_client.wlps_mutex));
	if (cnss_client.svc_running) {
		pthread_join(cnss_client.thread_id, NULL);
		cnss_client.svc_running = 0;
	}
	pthread_mutex_unlock(&(cnss_client.wlps_mutex));
...
void wlan_service_stop(void)
{
	...
	wsvc_printf_info("%s: WLPS service stopping",__func__);
